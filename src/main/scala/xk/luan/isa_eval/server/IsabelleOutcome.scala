...
case class IsabelleOutcome (stateId: String, result: String, proofLevel: Int, message: Option[String] = None) {
  def isSuccess: Boolean = result == "SUCCESS"
  def isFailure: Boolean = result == "ERROR"
  def isTimeout: Boolean = result == "TIMEOUT"
  def getMessage: String = message.getOrElse("")
}

case class IsabelleTrialResult (outcome: IsabelleOutcome, command: Option[String] = None) {
